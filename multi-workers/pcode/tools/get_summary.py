...
from pcode.tools.load_console_records import get_console_records, \
    get_checkpoint_args
...
def extract_list_of_records(list_of_records, conditions, interested_args,
    ...
    for raw_records in list_of_records:
        ...
        if use_log:
            print(('\twe are processing {} dataset on {}. We have {} workers, with {} local mini-batch size. \n' +
                  '\tWe are using constant lr={}, with weight decay={}.\n').format(
                args['data'], args['arch'], args['n_nodes'], args['batch_size'],
        ...
        # get parsed records
        info = get_parsed_console_records(console_records)
        records += [(args, info)]
...
def _get_parsed_console_te_records(events):
    ...
    events_steps = [e['current_epoch'] for e in events]

    return events_top1, events_times, events_steps
